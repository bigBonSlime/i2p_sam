...
public:
    ...
    template <typename T> void async_read(std::byte *dest, uint64_t bytes, T handler) {
        ...
        if (bytes <= buffer->size()) {
            is.read((char *)dest, bytes);
            buffer->consume(bytes);
            handler(errors::sam_error(), bytes);
        } else {
            std::size_t total = buffer->size();
            is.read((char *)dest, buffer->size());
            buffer->consume(buffer->size());
            boost::asio::async_read(
                this->socket, boost::asio::buffer(dest + buffer->size(), bytes - buffer->size()),
                [total, handler](const boost::system::error_code ec,
                                 std::size_t bytes_transferred) {
                    handler(ec ? (i2p_sam::errors::sam_error(
                                     i2p_sam::errors::error_codes::network_error, ec.what()))
    ...
    template <typename T> void async_read_line(T handler) {
        boost::asio::async_read_until(
            this->socket, *buffer, '\n', [=](const boost::system::error_code &ec, std::size_t) {
                if (!ec) {
                    std::string ret_str;
                    std::getline(std::istream(&(*buffer)), ret_str);
                    handler(std::move(ret_str), i2p_sam::errors::sam_error());
                } else {
                    handler(std::string(),
                            i2p_sam::errors::sam_error(i2p_sam::errors::network_error, ec.what()));
                }
            });
    }
    ...
    sam_socket(executor_type &ex) : socket(ex), buffer(new boost::asio::streambuf) {}
    template <typename executor_type>
    sam_socket(const executor_type &ex) : socket(ex), buffer(new boost::asio::streambuf) {}
            ...
            async_handshake(*sam_sock, handshake_params, [=](errors::sam_error ec) {
                if (!ec) {
                    ...
                    sam_sock->async_write(sam_create_request, [=](errors::sam_error ec, uint64_t) {
                        if (!ec) {
                            sam_sock->async_read_line([=](std::string s, errors::sam_error ec) {
                                if (!ec) {
                            ...
                            if (!ec) {
                                sam_sock->async_write(
                                    ...
                                    [sam_sock, handler](errors::sam_error ec, uint64_t) {
                                        if (!ec) {
                                            sam_sock->async_read_line([sam_sock, handler](
                                                                          std::string res,
                                                                          errors::sam_error ec) {
                                                if (!ec) {
                                                    errors::sam_error er = i2p_sam::get_result(res);
                                                    if (!er) {
...
public:
    ...
    void async_read_datagram(T handler) {
        this->socket.async_read_line([handler, socket = &this->socket](
                                         std::string s, i2p_sam::errors::sam_error ec) {
            if (!ec) {
                ...
                if (((s.size() >= sizeof(datagram_rec)) &&
                    ...
                    std::uint16_t from_port =
                        static_cast<std::uint16_t>(std::stoull(i2p_sam::get_value(s, "FROM_PORT")));
...
void async_handshake(sam_socket &sam_sock, const std::string &args, T handler) {
    std::string sam_request = std::string("HELLO VERSION MIN=" + i2p_sam::sam_version +
                                          " MAX=" + i2p_sam::sam_version + " ") +
                              args + std::string("\n");

    sam_sock.async_write(sam_request, [handler, &sam_sock](errors::sam_error err, uint64_t) {
        if (!err) {
            sam_sock.async_read_line([handler](std::string s, i2p_sam::errors::sam_error ec) {
                if (!ec) {
